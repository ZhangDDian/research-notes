...
## 四、行为实验佐证：LLM 社会模拟 61 轮

以上结论主要来自 benchmark 对比和工程经验。[llm-society](https://github.com/ZhangDDian/llm-society) 项目从行为心理学角度提供了更底层的解释——把多个 LLM agent 放进需要合作才能存活的小世界，61 轮对照实验（含统计检验和红蓝队交叉评审），回答"为什么多 agent 协调不工作"。

### 4.1 分布式协调 = 结构性不可能

给 agent 一个 `trade(对方, 我出X, 我要Y)` 原子交换动作，双方各自独立发出互补 trade 即成交。测了三种条件：同步匹配、挂单 3 天、免费沟通+挂单。

> **149 条消息讨论交换、26-68 次 trade 提议、挂单板上明确写着对方想换——匹配率 0%。**

不是"效果不好"，是完全不工作。模型能做单体决策，不能做分布式协调。这比 benchmark 上"单 agent 略优"更有冲击力——多 agent 协调在行为层面就是做不到。

### 4.2 完美主义陷阱：多 agent 架构本身会让情况更糟

给模型一个"更优但需要协调"的选项（trade），它会**放弃所有有效的替代方案**——give 次数直接归零。知道合成最优（18 点）就不吃手里的次优（6 点），宁可饿死。

这解释了为什么多 agent 系统经常比单 agent 更差——不只是加了通信开销，多 agent 架构本身暗示了"应该协调"，导致模型放弃了单体就能完成的简单动作。

### 4.3 语境顺从者：同构多 agent 为什么是伪命题

核心发现：**LLM 不算账，读空气。** 同一套经济规则，把"你们是村民"换成"你们是竞争者"，give:eat 从 31:0 翻转到 3:7。行为不由理性计算决定，由语境暗示决定。

这意味着给同一个 LLM 套上 CEO/CTO/PM 角色，它只会**表演角色而不是解决问题**——实验直接证明文学人格描写降低合作率（p=0.025）。

### 4.4 零心智理论：agent 间协商是无效的

让同模型两个 agent 对弈，一个说真话或撒谎，另一个判断真假。结果：事实题 100% 准确（查知识库），观点题 50%（和抛硬币一样），策略题 100% 判为谎言（硬编码先验）。

**LLM 没有任何形式的心智理论。** 多 agent 系统中的"协商""讨论""达成共识"全是无效动作——模型不是在理解对方意图，是在做模式匹配。

### 4.5 设计启示：单方面触发 > 协调触发

实验结论直接支持 subagent 模式的设计哲学：

- 不要设计需要多 agent "各自独立做出互补选择"的协调机制——做不到
- 用"单方面触发 + 系统自动完成"（如 A give 后系统自动匹配回赠）
- 动作命名不是标签，是行为指令——叫"背叛"它就背叛，叫"选项 B"它就正常思考
- 想让 agent 做"自私但合理的事"，把它框定为义务——"维护自身状态是对团队负责"

> 完整实验记录、代码和数据：[github.com/ZhangDDian/llm-society](https://github.com/ZhangDDian/llm-society)
...
### 一手实验
- [LLM 社会模拟实验 — llm-society](https://github.com/ZhangDDian/llm-society)（2026.05，61 轮对照实验，多 agent 协调匹配率 0%，语境顺从者结论）
